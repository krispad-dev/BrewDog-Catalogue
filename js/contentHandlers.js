...
export const contentHandler = () => {
  ...
  const getRandomData = async () => {
    for (let i = 0; i < 7; ++i) {
      // Loop for collecting random get-requests
  ...
  searchInput.addEventListener('keyup', (e) => {
    ...
    const getSearchedData = async () => {
      const res = await fetch(
        `https://api.punkapi.com/v2/beers?${searchType}=${keyInput}&per_page=7`
      );
  ...
  beers.forEach((beer, i) => {
    ...
    gridContainer.innerHTML += `
        <section style="background-image: url(${
          image_url ? image_url : placeholderImg
        });" class="box-${i + 1} box">
            <div class="box-text-wrapper">
                ...
                <div class="hidden">
                    ...
                    <ul class="food-pairing">${liLoop(food_pairing).join(' ')}</ul>
    ...
    boxes.forEach((box, i) => {
      ...
      box.addEventListener('click', () => {
        // Toggles where you can click on the screen, adding hidden information about cards and presents single bigger box.
...
// Creates list item with array item inside and returns
const liLoop = (arr) => {
